...
import { getZapEdus, deleteZapEduById } from 'apiSdk/zap-edus';
import { ZapEduInterface } from 'interfaces/zap-edu';

type ColumnType = ColumnDef<ZapEduInterface, unknown>;

interface ZapEduListPageProps {
...
export function ZapEduListPage(props: ZapEduListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<ZapEduInterface>>(
    () => `/zap-edus?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: ZapEduInterface) => {
    if (hasAccess('zap_edu', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/zap-edus/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/zap-edus/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('zap_edu', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/zap-edus/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('zap_edu', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
