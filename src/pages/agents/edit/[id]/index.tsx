...
import { getAgentById, updateAgentById } from 'apiSdk/agents';
...
function AgentEditPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<AgentInterface>(
    () => (id ? `/agents/${id}` : null),
    () => getAgentById(id),
  );
  ...
  const handleSubmit = async (values: AgentInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateAgentById(id, values);
      mutate(updated);
      resetForm();
      router.push('/agents');
    } catch (error: any) {
